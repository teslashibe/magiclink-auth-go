...
// Sender sends email via SendGrid.
type Sender struct {
	apiKey      string
	fromAddress string
	client      *http.Client
...
func (s *Sender) Send(ctx context.Context, to, subject, htmlBody string) error {
	...
	if err != nil {
		return fmt.Errorf("invalid sendgrid from address: %w", err)
	}
	...
	payload := map[string]any{
		"personalizations": []any{
			map[string]any{
				"to": []any{
					map[string]string{"email": strings.TrimSpace(to)},
				},
			},
		},
		"from":    from,
	...
	if resp.StatusCode >= 300 {
		...
		return fmt.Errorf("sendgrid %d: %s", resp.StatusCode, strings.TrimSpace(string(respBody)))
	}
	return nil
}
